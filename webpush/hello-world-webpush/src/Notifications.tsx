      ...
      <PageHeader
        ...
        avatar={
          <>
            {' '}
            |{' '}
            <Button onClick={() => this.setState({ reconfig: true })}>
              Configure
            </Button>{' '}
          </>
        }
          ...
          if (this.state?.notifications?.length! > 0) {
            return this.state!.notifications!.map((item: Notification) => {
              return {
                cells: [
                  { title: getIcon(item) },
                  { title: moment(item.date).format('DD-MM-YYYY HH:mm:ss') },
                  { title: item.text },
                ],
              };
            });
            ...
            return renderEmptyTable({
              state: 'INFO',
              title: 'No push messages have been received so far',
              message:
                'Please log into your Aerogear UnifiedPush Server and send some message',
            });
          }
        case STATE.ERROR:
          return renderEmptyTable({
            state: 'ERROR',
            title: 'An error occurred registering your application',
            message: this.state.error!,
          });
        default:
          return renderEmptyTable({
            state: 'WARNING',
            title: 'Web Application not registered',
            message: `Please click on 'Register to UPS' to connect to the ups.`,
          });
      }
    };
    const columns = [
